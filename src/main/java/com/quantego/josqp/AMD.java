...
public class AMD {
	...
	public enum Status {
		AMD_INVALID, AMD_OK, AMD_OK_BUT_JUMBLED
	}
	
	public static final int EMPTY = -1;
	public static final int FLIP(int i) {return -(i)-2;}
	public static final int UNFLIP(int i) {return ((i < EMPTY) ? FLIP (i) : (i));}
	/* contents of Control */
	public static final int AMD_DENSE = 0  ;          /* "dense" if degree > Control [0] * sqrt (n) */
	public static final int AMD_AGGRESSIVE = 1 ;   /* do aggressive absorption if Control [1] != 0 */

	/* default Control settings */
	public static final double AMD_DEFAULT_DENSE = 10.0 ;         /* default "dense" degree 10*sqrt(n) */
	public static final int AMD_DEFAULT_AGGRESSIVE = 1 ;   /* do aggressive absorption by default */

	/* contents of Info */
	public static final int AMD_STATUS = 0  ;         /* return value of amd_order and amd_l_order */
	public static final int AMD_N = 1  ;              /* A is n-by-n */
	public static final int AMD_NZ = 2  ;    /* number of nonzeros in A */
	public static final int AMD_SYMMETRY = 3 ;        /* symmetry of pattern (1 is sym., 0 is unsym.) */
	public static final int AMD_NZDIAG = 4  ;         /* # of entries on diagonal */
	public static final int AMD_NZ_A_PLUS_AT = 5;  /* nz in A+A' */
	public static final int AMD_NDENSE = 6  ;         /* number of "dense" rows/columns in A */
	public static final int AMD_MEMORY = 7  ;         /* amount of memory used by AMD */
	public static final int AMD_NCMPA = 8  ;          /* number of garbage collections in AMD */
	public static final int AMD_LNZ = 9  ;   /* approx. nz in L, excluding the diagonal */
	public static final int AMD_NDIV = 10 ;           /* number of fl. point divides for LU and LDL' */
	public static final int AMD_NMULTSUBS_LDL = 11; /* number of fl. point (*,-) pairs for LDL' */
	public static final int AMD_NMULTSUBS_LU = 12 ; /* number of fl. point (*,-) pairs for LU */
	public static final int AMD_DMAX = 13   ;          /* max nz. in any column of L, incl. diagonal */
	
	
		
	
	public static AMD.Status amd_order(int n, int[] Ap, int[] Ai, int[] P) {
	    ...
	    if (Ai == null || Ap == null || P == null || n < 0) {
	    	return (Status.AMD_INVALID) ;	    /* arguments are invalid */
	    }

	    if (n == 0) {
	    	return (Status.AMD_OK) ;	    /* n is 0 so there's nothing to do */
	    }
	    ...
	    if (status == Status.AMD_INVALID) {
	    	return (Status.AMD_INVALID) ;	    /* matrix is invalid */
	    }
	    ...
	    if (status == Status.AMD_OK_BUT_JUMBLED) {
	...
	public static AMD.Status amd_valid(int n_row, int n_col, int[] Ap, int[] Ai) {

		int nz, j, p1, p2, ilast, i, p; 
		AMD.Status result = Status.AMD_OK;
	    ...
	    for (j = 0 ; j < n_col ; j++){
			...
			for (p = p1 ; p < p2 ; p++) {
			    ...
			    if (i <= ilast) {
			    	...
			    	result = Status.AMD_OK_BUT_JUMBLED;
			    }
			    ilast = i ;
			}
	    }
	...
	public static void amd_preprocess(int n,		/* input matrix: A is n-by-n */
		   int[] Ap,	/* size n+1 */
		   int[] Ai,	/* size nz = Ap [n] */
	...
	)
	{
	    ...
	    amd_2 (n, Pe, Iw, Len, iwlen, pfree, Nv, Pinv, P, Head, Elen, Degree, W) ;
	}
	 ...
	 */
		...
		nvi, nvj, nvpiv, slenme, wbig, we, wflg, wnvi,
		dense, aggressive ;
	    boolean ok;
	...
	*/
	...
	public static void amd_postorder(
	...
	public static int amd_post_tree (
	    ...
	    int[] Child,		/* input argument of size nn, undefined on
					 ...
					 * i in the tree. */
	    int[] Sibling,	/* input argument of size nn, not modified.
